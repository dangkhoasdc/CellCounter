...
class Framework(AbsFramework):
    ...
    def preprocess_pca(self, image):
        """
        resize an image and flatten it
        """
        result = cv2.resize(image, (25, 25))
    ...
    def train(self, image_lst, loc_lst, viz=False):
        ...
        print "Get data done"
        training_data = [self.preprocess_pca(im) for im in data]
        training_data = np.vstack(training_data)
        labels = np.float32(labels)
        self._pca.fit(training_data)
        eigencells = self._pca.components_.reshape((self._n_components , 25, 25))
        # for eigencell in eigencells:
            # com.debug_im(eigencell)
        training_data = self._pca.transform(training_data)
